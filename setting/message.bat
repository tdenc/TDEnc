...
set PAUSE_MESSAGE1=Enterキーを押してね
...
set MUX_ALERT1=.nvvファイルはMUXモードに対応してないわ
set MUX_ALERT2=NiVEで音声と一緒にしてからnvvファイルのみドラッグ＆ドロップしてね！
set CACHE_ANNOUNCE=テンポラリファイル作成中～♪（容量が大きいと時間かかるわよ）
set CACHE_END=テンポラリファイルを作成完了～♪
set SWF_ALERT=swfファイルはversion1.01からサポート対象外なの
set NVV_ALERT1=warpsharp.dll改造版がtoolフォルダに入ってないわよ！
set NVV_ALERT2=nicoAE.NVVInOut.UnmanagedUtils.dllがtoolフォルダに入ってないわよ！
set NVV_ALERT3=nvvinput.auiがtoolフォルダに入ってないわよ！
set VIDEO_ENC_ERROR=映像のエンコードに失敗したわ・・・
set AVS_MESSAGE1=Avisynthのレジストリが登録されていないわ
set AVS_MESSAGE2=レジストリエディタを起動するから実行を許可してね
set AVS_MESSAGE3=その後すべての質問に「はい」と答えていってね
set AVS_ERROR1=Avisynthのレジストリ登録に失敗・・・ごめんなさい・・・
set AVS_ERROR2=Avisynthを別途インストールしてからもう一度使ってみてね！
set AVS_ERROR3=（Archivesフォルダのavisynth_258.exeをダブルクリック）
...
set PRESET_LIST7=ユーザー定義プリセット（デフォルトは激重プリセット、PCが危険）
